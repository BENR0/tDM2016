...
```{r overview, echo = FALSE, fig.cap = figNum("Ueberblick der Zeitreihe NO3")}
...
```{r GMM plot, echo = FALSE,fig.cap = figNum("GMM für tranformierte NO3 ohne Nullen, 3 Gauss")}
Werte1 <- list(Means = c(7.029219, 17.786148, 29.106267), 
           SDs = c(5.08932, 4.50063, 5.69700), 
           Weights = c( 0.176, 0.348, 0.490))
...
Die Anpassung sieht im QQ-Plot bis auf die Werte am Rand gut aus. Zur Verifizierung verwenden wir den Chi-Quadrat-Test.
...
Der P-Value ist mit einem Wert von  groß, H0 wrid nicht abgeleht. Anpassung gut.
...
***
### ARCH/GARCH
Wir betrachten die Differenzreihe, da die Zeitreihe stationär sein muss.
...
Die Nullhypothese wird abgelehnt, da es einen ARCH Effekt gibt.

Die Parameter werden hier durch Ausprobieren bestimmt. Anschließendwird mit Hilfe des AIC-Kriteriums der beste Parameter bestimmt.
...
```{r Arch/ Garch plot, echo = FALSE, fig.cap = figNum("Güte des GARCH Modelles mit unterschiedlichen Parametern")}
plot(aic, type = "l")
points(aic)
...
Box-Ljung Test überprüft, ob innerhalb der Residuen eine Autokorrelation existiert. In den Residuen dieses Datensatzes ist keine Autokorrekation zu finden.
...
Es sind insgesamt `r length(Data[,1])` Werte, die wie bei unserem vorherigen Datensatz NN03 alle 30 Minuten von 13:30 Uhr (3.1.2014) bis 8 Uhr (14.12.2015) gemessen worden sind.
...
```{r overview_Precip, echo = FALSE, fig.cap = figNum("Ueberblick der Zeitreihe Precipitation")}
...
Wir haben die Nullen und die 0,1 Werte als seperate Gruppen aus dem Datensatz genommen und betrachten nun die restlichen Daten.
...
Die restlichen Daten sind weiterhin nicht normalverteilt, weshalb wir die Daten transformieren. Da der Wertebereich sehr klein ist, werden auch diese Daten mit einem Faktor multipliziert.
...
Transformatierte Daten^(-3)

```{r inspect var trafo 2_Precip, echo = FALSE,fig.cap = figNum("Visuelle Inspektion transformierter Precipitation Daten ohne 0,0 und 0,1.")}
...
```{r GMM plot_Precip, echo = FALSE,fig.cap = figNum("GMM für tranformierte Precipitation Daten ohne 0,0 und 0,1, 3 Gauss"),eval = eval_block}
Werte2 <- list(Means = c(126.0190, 429.6519, 712.7960), 
           SDs = c(121.8712, 122.5664, 135.4684), 
           Weights = c( 0.343, 0.224, 0.553))
PlotMixturesAndBoundaries(transNO3, Werte1$Means, Werte1$SDs, Werte1$Weights,SingleGausses=T, lwd=3)
...
P-Wert ist sehr groß. Da der Stichprobenumfang sehr klein ist,`r length(transPrec)` wird $H_0$ nicht abgelehnt und wir nehmen die Anpassung an.
