...
```{r problem specific functions, echo = FALSE}
...
```{r difference_plot, echo = FALSE, htmlcap = figNum("Differenz des Datensatzes")}
...
PlotMixturesAndBoundaries(coredata(sixthroot), GMMRadMeans, GMMRadSDs, GMMRadWeights, SingleGausses = TRUE)
```
...
```{r 1, echo = FALSE, eval = eval_block}

```

```{r 2, echo = FALSE, eval = eval_block}

```


```{r Jiachun NO3 data, child = "P2_child_script_Jiachun.Rmd", eval = TRUE}
```
